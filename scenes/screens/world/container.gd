...
func spawn_players( country : String, goal : Goal ) -> Array[ Player ]:
	...
	for i in players.size():
		...
		var player := spawn_player( player_position, kickoff_position, goal, target_goal, player_data, country )
		player_nodes.append( player )
		add_child( player )
	return player_nodes


func spawn_player( player_position : Vector2, kickoff_position : Vector2, goal : Goal, target_goal : Goal, player_data : PlayerResource, country : String ) -> Player:
	var player : Player = PLAYER_PREFAB.instantiate()
	player.initialize( player_position, kickoff_position, ball, goal , target_goal, player_data, country )
...
func setup_control_schemes() -> void:
	...
	if GameManager.is_coop():
		var player_squad := squad_left if squad_left[0].country == p1_country else squad_right
		...
		var p1_squad := squad_left if squad_left[0].country == p1_country else squad_right
		var p2_squad := squad_left if p1_squad == squad_right else squad_right
