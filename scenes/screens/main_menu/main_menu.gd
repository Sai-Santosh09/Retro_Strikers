...
func _process( _delta: float ) -> void:
	if is_active:
		if KeyUtil.is_action_just_pressed( Player.ControlScheme.P1, KeyUtil.Action.UP ):
			change_selected_index( current_selected_index - 1 )
		elif KeyUtil.is_action_just_pressed( Player.ControlScheme.P1, KeyUtil.Action.DOWN ):
			change_selected_index( current_selected_index + 1 )
		elif KeyUtil.is_action_just_pressed( Player.ControlScheme.P1, KeyUtil.Action.SHOOT ):
			submit_selection()
