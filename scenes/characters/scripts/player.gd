...
func _ready() -> void:
	...
	var initial_position := kickoff_position if country == GameManager.current_match.country_left else spawn_position
...
func initialize( context_position : Vector2, context_kickoff_position : Vector2, context_ball : Ball, context_goal : Goal, context_target_goal : Goal, context_player_data : PlayerResource, context_country : String ) -> void:
	...
	Left_or_Right = Vector2.LEFT if target_goal.position.x < position.x else Vector2.RIGHT
...
func get_hurt( hurt_origin : Vector2 ) -> void:
	switch_states( Player.State.HURT, PlayerStateData.build().set_hurt_direction( hurt_origin ) )
